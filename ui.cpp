...
static GRSurface *gBackgroundIcon[NUM_BACKGROUND_ICONS];
static GRSurface **gInstallationOverlay;
static GRSurface **gProgressBarIndeterminate;
static GRSurface *gProgressBarEmpty;
static GRSurface *gProgressBarFill;
...
static void draw_install_overlay_locked(int frame)
{
	if (gInstallationOverlay == NULL)
		return;
	GRSurface *surface = gInstallationOverlay[frame];
...
static void draw_background_locked(int icon)
{
	...
	if (icon) {
		GRSurface *surface = gBackgroundIcon[icon];
		int iconWidth = gr_get_width(surface);
		int iconHeight = gr_get_height(surface);
...
static void draw_status_locked()
{
	...
	if (status_modified) {
		...
		int dy =
		    CHAR_HEIGHT + height + (3 * gr_fb_height() +
					    iconHeight) / 4;
...
static void draw_progress_locked()
{
	...
	if (gProgressBarType != PROGRESSBAR_TYPE_NONE) {
		...
		if (gProgressBarType == PROGRESSBAR_TYPE_NORMAL) {
			float progress =
			    gProgressScopeStart +
			    gProgress * gProgressScopeSize;
			int pos = (int)(progress * width);

			if (pos > 0) {
				gr_blit(gProgressBarFill, 0, 0, pos, height, dx,
					dy);
			}
			if (pos < width - 1) {
		...
		if (gProgressBarType == PROGRESSBAR_TYPE_INDETERMINATE) {
			static int frame = 0;
			gr_blit(gProgressBarIndeterminate[frame], 0, 0, width,
				height, dx, dy);
			frame =
			    (frame + 1) % ui_parameters.indeterminate_frames;
		}
	}
}
...
static void draw_screen_locked(void)
{
	...
	if (show_menu) {
		...
		gr_fill(0, (menu_top + menu_sel) * CHAR_HEIGHT,
			gr_fb_width(),
			(menu_top + menu_sel + 1) * CHAR_HEIGHT + 1);
...
static void *progress_thread(void *cookie)
{
	...
	for (;;) {
		...
		if (gCurrentIcon == BACKGROUND_ICON_INSTALLING &&
			...
			gInstallingFrame =
			    (gInstallingFrame +
			     1) % ui_parameters.installing_frames;
			redraw = 1;
		}
...
void mui_init(void)
{
	...
	for (i = 0; BITMAPS[i].name != NULL; ++i) {
		int result =
		    res_create_display_surface(BITMAPS[i].name,
					       BITMAPS[i].surface);
		if (result < 0) {
	...
	gProgressBarIndeterminate = (GRSurface**) malloc(
					ui_parameters.indeterminate_frames * sizeof(GRSurface*));
	for (i = 0; i < ui_parameters.indeterminate_frames; ++i) {
		char filename[40];
		// "indeterminate01.png", "indeterminate02.png", ...
		sprintf(filename, "indeterminate%02d", i + 1);
		int result =
		    res_create_display_surface(filename,
					       gProgressBarIndeterminate + i);
		if (result < 0) {
			printf("Missing bitmap %s\n(Code %d)\n", filename,
			       result);
		}
	...
	if (ui_parameters.installing_frames > 0) {
		gInstallationOverlay = (GRSurface**) malloc(
						ui_parameters.installing_frames * sizeof(GRSurface*));
...
void mui_infotext(const char *infodata)
{
	...
	pthread_mutex_lock(&gTextMutex);
	info_row = 0;
	for (str = idata, info_row = 0; info_row < MAX_ROWS;
	     str = NULL, info_row++) {
		token = strtok_r(str, "\n", &saveptr);
		if (!token) {
...
int mui_wait_key()
{
	...
	if (key_queue_len > 0) {
		key = key_queue[0];
		memcpy(&key_queue[0], &key_queue[1],
		       sizeof(int) * --key_queue_len);
	}
	pthread_mutex_unlock(&key_queue_mutex);
	return key;
}
