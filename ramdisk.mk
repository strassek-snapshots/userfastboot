...
$(USERFASTBOOT_RAMDISK): \
	...
	$(hide) $(ACP) -f $(TARGET_RECOVERY_ROOT_OUT)/default.prop $(USERFASTBOOT_ROOT_OUT)/default.prop
...
ifneq ($(USERFASTBOOT_NO_GUI),true)
	...
	$(foreach EXTRA,$(USERFASTBOOT_EXTRAS), \
		$(hide) $(ACP) -r $(EXTRA) $(USERFASTBOOT_ROOT_OUT)/)
...
$(USERFASTBOOT_BOOTIMAGE): \
	...
	$(BOOT_SIGNER) /fastboot $(USERFASTBOOT_BOOTIMAGE) $(PRODUCTS.$(INTERNAL_PRODUCT).PRODUCT_VERITY_SIGNING_KEY).pk8 $(PRODUCTS.$(INTERNAL_PRODUCT).PRODUCT_VERITY_SIGNING_KEY).x509.pem $(USERFASTBOOT_BOOTIMAGE)
